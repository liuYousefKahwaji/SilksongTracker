"""Minimal pure-Python AES-256 ECB decryption.

Only used when neither `cryptography` nor `pycryptodome` is installed, so the
tracker keeps working on a bare Python install. Decrypt-only, 256-bit keys.
"""
